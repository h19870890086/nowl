...
public class SearchSyncServiceImpl implements SearchSyncService {
    ...
    private void doSyncGoods(Long productId) {
        ...
        // 仅审核通过且处于在售状态的商品才同步到ES；不满足条件的从ES中删除
        boolean shouldBeInEs = isReviewPassed(goods.getReviewStatus())
                && TradeStatus.ON_SALE.getCode().equals(goods.getTradeStatus());

        if (!shouldBeInEs) {
            goodsSearchRepository.deleteById(productId);
            log.info("商品不满足上架条件，已从ES移除: productId={}, reviewStatus={}, tradeStatus={}",
                    productId, goods.getReviewStatus(), goods.getTradeStatus());
            return;
        }
    ...
    public void fullSync() {
        ...
        try {
            ...
            while (true) {
                ...
                // 仅同步审核通过(1-AI通过, 2-人工通过)且处于在售状态(0)的商品
                wrapper.in(GoodsInfo::getReviewStatus, ReviewStatus.AI_PASSED.getCode(), ReviewStatus.MANUAL_PASSED.getCode())
