...
public class CsrfFilter extends OncePerRequestFilter {
    ...
    /**
     * CSRF宽松检查路径：登录/注册等公开接口放行X-Requested-With检查，
     * 兼容部分浏览器（QQ/联想/UC等）可能不发该请求头的问题。
     */
    private static final java.util.Set<String> CSRF_RELAXED_PATHS = java.util.Set.of(
            "/auth/login", "/auth/register", "/auth/send-sms", "/auth/send-email-code",
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        boolean isRelaxedPath = CSRF_RELAXED_PATHS.contains(path);

        // 登录/注册等公开接口：放宽X-Requested-With检查（兼容QQ/联想/UC等浏览器）
        // 但仍保留Origin/Referer校验防止跨站请求
        String requestedWith = request.getHeader(REQUESTED_WITH_HEADER);
        if (!REQUESTED_WITH_VALUE.equals(requestedWith) && !isRelaxedPath) {
