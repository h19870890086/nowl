...
public class AdminErrandDomainService {
    ...
    /**
     * 手动重同步单个跑腿任务到ES（管理员触发，用于修复ES数据不一致）
     */
    public void resyncErrandToEs(Long taskId) {
        ErrandTask task = errandTaskMapper.selectById(taskId);
        if (task == null) {
            throw new BusinessException("跑腿任务不存在");
        }
        boolean shouldBeInEs = (ReviewStatus.AI_PASSED.getCode().equals(task.getReviewStatus())
                || ReviewStatus.MANUAL_PASSED.getCode().equals(task.getReviewStatus()))
                && !ErrandStatus.CANCELLED.getCode().equals(task.getTaskStatus());

        if (shouldBeInEs) {
            rocketMQTemplate.convertAndSend(ERRAND_SYNC_TOPIC, ErrandSyncMessage.updateMessage(taskId));
            log.info("管理员手动同步跑腿到ES(update): taskId={}", taskId);
        } else {
            rocketMQTemplate.convertAndSend(ERRAND_SYNC_TOPIC, ErrandSyncMessage.deleteMessage(taskId));
            log.info("管理员手动同步跑腿到ES(delete): taskId={}, reviewStatus={}, taskStatus={}",
                    taskId, task.getReviewStatus(), task.getTaskStatus());
        }
    }
