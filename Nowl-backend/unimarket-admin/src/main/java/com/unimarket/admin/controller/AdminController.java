...
public class AdminController {
    ...
    /**
     * 手动重同步单个商品到ES（管理员接口，用于修复ES数据不一致）
     */
    @PostMapping("/goods/resync")
    @PreAuthorize("@iamAuth.hasPerm(authentication.principal.userId, 'admin:goods:list:view')")
    public Result<Void> resyncGoods(@RequestParam Long goodsId) {
        adminService.resyncGoodsToEs(goodsId);
        return Result.success();
    }

    /**
     * 手动重同步单个跑腿任务到ES（管理员接口，用于修复ES数据不一致）
     */
    @PostMapping("/errand/resync")
    @PreAuthorize("@iamAuth.hasPerm(authentication.principal.userId, 'admin:errand:list:view')")
    public Result<Void> resyncErrand(@RequestParam Long taskId) {
        adminService.resyncErrandToEs(taskId);
        return Result.success();
    }
}
