...
public class AdminGoodsDomainService {
    ...
    /**
     * 手动重同步单个商品到ES（管理员触发，用于修复ES数据不一致）
     */
    public void resyncGoodsToEs(Long goodsId) {
        GoodsInfo goods = goodsInfoMapper.selectById(goodsId);
        if (goods == null) {
            throw new BusinessException("商品不存在");
        }
        boolean shouldBeInEs = (ReviewStatus.AI_PASSED.getCode().equals(goods.getReviewStatus())
                || ReviewStatus.MANUAL_PASSED.getCode().equals(goods.getReviewStatus()))
                && TradeStatus.ON_SALE.getCode().equals(goods.getTradeStatus());

        if (shouldBeInEs) {
            sendGoodsSyncAfterCommit(GoodsSyncMessage.updateMessage(goodsId));
            log.info("管理员手动同步商品到ES(update): goodsId={}", goodsId);
        } else {
            sendGoodsSyncAfterCommit(GoodsSyncMessage.deleteMessage(goodsId));
            log.info("管理员手动同步商品到ES(delete): goodsId={}, reviewStatus={}, tradeStatus={}",
                    goodsId, goods.getReviewStatus(), goods.getTradeStatus());
        }
    }
