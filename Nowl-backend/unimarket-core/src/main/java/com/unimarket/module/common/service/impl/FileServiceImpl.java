...
public class FileServiceImpl implements FileService {
    ...
    private static final String USER_UPLOAD_PREFIX = "%d/";
    ...
    private String uploadDirect(Long userId, MultipartFile file) throws IOException {
        ...
        String uploadedPath = cosUtils.upload(file, keyPrefix);
        // COS 返回的是完整 HTTPS URL，直接返回即可
        if (uploadedPath.startsWith("http://") || uploadedPath.startsWith("https://")) {
            return uploadedPath;
        }
        // 本地存储返回相对路径，需确保以 /uploads/ 开头与静态资源映射匹配
        if (!uploadedPath.startsWith("/uploads/")) {
            uploadedPath = "/uploads/" + uploadedPath.replaceFirst("^/+", "");
        }
        return uploadedPath;
    }
