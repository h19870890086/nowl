...
public class GoodsAuditServiceImpl implements GoodsAuditService {
    ...
    /**
     * 发送ES删除同步消息（用于待人工复核/审核失败场景，确保商品从搜索结果中移除）
     */
    private void sendDeleteSyncMessage(Long goodsId) {
        GoodsSyncMessage message = GoodsSyncMessage.deleteMessage(goodsId);
        if (TransactionSynchronizationManager.isSynchronizationActive()) {
            TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
                @Override
                public void afterCommit() {
    ...
    private void handlePendingHumanReview(GoodsInfo goods, String reason, String timeStr) {
        ...
        // 待人工复核期间从ES移除，审核通过后由人工审核流程重新同步
        sendDeleteSyncMessage(goods.getProductId());
