...
public interface UserInfoMapper extends BaseMapper<UserInfo> {
    ...
    /**
     * 原子更新用户余额（正数为加钱，负数为扣钱）
     * 避免 read-modify-write 导致的并发丢失更新问题
     */
    @Update("UPDATE user_info SET money = money + #{amount}, update_time = NOW() WHERE user_id = #{userId}")
    int addMoney(@Param("userId") Long userId, @Param("amount") java.math.BigDecimal amount);
}
