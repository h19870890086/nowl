...
public class GoodsServiceImpl implements GoodsService {
    ...
    // 默认占位图(base64 SVG)，商品无图片时使用，不依赖外部CDN
    ...
    public GoodsDetailVO getDetail(Long productId, Long currentUserId) {
        ...
        vo.setImage(resolveProductImage(goodsInfo));
        if (seller != null) {
    ...
    /**
     * 解析商品展示图片：优先用image封面，为空时从imageList JSON数组中提取第一张
     */
    private String resolveProductImage(GoodsInfo goods) {
        if (StrUtil.isNotBlank(goods.getImage())) {
            return goods.getImage();
        }
        if (StrUtil.isNotBlank(goods.getImageList())) {
            try {
                java.util.List<String> images = cn.hutool.json.JSONUtil.toList(goods.getImageList(), String.class);
                if (!images.isEmpty()) {
                    return images.get(0);
                }
            } catch (Exception e) {
                log.warn("解析imageList失败: productId={}", goods.getProductId(), e);
            }
        }
    ...
    public void publish(Long userId, GoodsPublishDTO dto) {
        ...
        // 如果封面图为空，从 imageList JSON 数组中提取第一张作为封面
        if (StrUtil.isBlank(goodsInfo.getImage()) && StrUtil.isNotBlank(goodsInfo.getImageList())) {
            try {
                java.util.List<String> images = cn.hutool.json.JSONUtil.toList(goodsInfo.getImageList(), String.class);
                if (!images.isEmpty()) {
                    goodsInfo.setImage(images.get(0));
                }
            } catch (Exception e) {
                log.warn("解析imageList失败，跳过封面图提取: {}", e.getMessage());
            }
        }
    ...
    private List<GoodsVO> convertToVOList(List<GoodsInfo> goodsList, Long currentUserId) {
        ...
        for (GoodsInfo goods : goodsList) {
            ...
            // 统一解析图片：优先image封面，为空时从imageList提取，都没有则用占位图
            vo.setImage(resolveProductImage(goods));
