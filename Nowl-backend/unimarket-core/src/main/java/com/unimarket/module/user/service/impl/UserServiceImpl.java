...
import com.unimarket.utils.EmailUtils;
...
public class UserServiceImpl extends ServiceImpl<UserInfoMapper, UserInfo> implements UserService {
    ...
    public void register(UserRegisterDTO dto) {
        // 0. 校验邮箱验证码
        String cacheCode = redisCache.get("email:code:" + dto.getEmail(), String.class);
        if (cacheCode == null || !cacheCode.equals(dto.getCode())) {
            throw new BusinessException("验证码错误或已失效");
        }
        // 验证通过后删除验证码，防止重复使用
        redisCache.delete("email:code:" + dto.getEmail());
        ...
        // 2. 创建用户（邮箱存入数据库，user_info表可能需要加字段或复用phone字段）
    ...
    public void sendEmailCode(String email) {
        ...
        String code = cn.hutool.core.util.RandomUtil.randomNumbers(6);
        // 发送邮件
        boolean success = emailUtils.sendVerifyCode(email, code);
        if (!success) {
        ...
        // 设置限流Key，60秒有效
        redisCache.set(limitKey, "1", 60);
        log.info("邮件验证码已发送，email={},code={}", email, code);
    }
    ...
    private String resolveRequestDevice() {
        ...
        // 截断超长UA（QQ浏览器等可达250+字符），防止数据库列溢出
