...
// SearchableSelect 学校选项
const schoolSelectOptions = computed(() =>
  props.schoolList.map(s => ({ label: s.schoolName, value: s.schoolCode }))
)
const schoolCode = computed({
  get: () => props.filterSchoolCode || '',
  set: (val: string | number | undefined) => emit('update:filterSchoolCode', (val ?? '') as string),
})
...
<template>
  <div class="admin-panel">
    ...
    <div v-else class="overflow-x-auto rounded-2xl border border-warm-100">
      <table class="w-full">
        <thead>
          <tr class="bg-gradient-to-r from-slate-50 to-emerald-50/50">
