...
// 转换为 SearchableSelect 所需格式
const schoolSelectOptions = computed(() =>
  schoolOptions.value.map(s => ({ label: s.schoolName, value: s.schoolCode }))
)
const campusSelectOptions = computed(() =>
  campusList.value.map(c => ({ label: c.campusName, value: c.campusCode }))
)
const gradeSelectOptions = gradeOptions.map(g => ({ label: g, value: g }))
