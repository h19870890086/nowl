...
const handleOrderAction = async (
  ...
  successMsg: string,
  newStatus?: number, // 操作成功后的新状态，用于即时更新UI
) => {
  ...
  try {
    ...
    // 乐观更新：立即修改本地订单状态，避免UI在刷新完成前显示旧按钮
    if (newStatus !== undefined) {
      const updateLocalOrder = (list: OrderInfo[]) => {
        const target = list.find(o => o.orderId === orderId)
        if (target) {
          target.orderStatus = newStatus
        }
      }
      updateLocalOrder(ordersCache.value.buy)
      updateLocalOrder(ordersCache.value.sell)
      updateLocalOrder(orders.value)
    }
    void refresh()
  } catch (error) {
    if (error !== 'cancel') {
...
const handlePay = (order: OrderInfo) => {
  ...
  void handleOrderAction(order.orderId, '确认支付该订单？', payOrder, '支付成功', OrderStatus.PENDING_DELIVERY)
}
...
const handleDeliver = (order: OrderInfo) => {
  ...
  void handleOrderAction(order.orderId, '确认已发货？', deliverOrder, '发货成功', OrderStatus.PENDING_RECEIVE)
}
...
const handleConfirm = (order: OrderInfo) => {
  ...
  void handleOrderAction(order.orderId, '确认收货？', confirmOrder, '确认收货成功', OrderStatus.COMPLETED)
}
...
const handleCancel = (order: OrderInfo) => {
  ...
  void handleOrderAction(order.orderId, '确认取消该订单？', cancelOrder, '订单已取消', OrderStatus.CANCELLED)
}
...
// 内联SVG占位图(base64)，避免依赖外部服务被墙
const INLINE_PLACEHOLDER = 'data:image/svg+xml;base64,'

const productImage = (order: OrderInfo) =>
  normalizeMediaUrl(order.productImage) || INLINE_PLACEHOLDER
