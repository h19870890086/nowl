...
// UUID v4 生成（纯Math.random实现，兼容所有浏览器包括QQ/联想/UC）
...
const fetchCaptcha = async () => {
  try {
    ...
    console.error('[Captcha]', msg, err)
    ElMessage.error('获取验证码失败: ' + (msg.length > 30 ? msg.substring(0, 30) + '...' : msg))
  }
}
...
onMounted(() => {
  // 清除可能残留的登录状态（防止localStorage持久化导致的异常）
  userStore.logout({ notifyServer: false }).finally(() => {
    fetchCaptcha()
  })
})
</script>
