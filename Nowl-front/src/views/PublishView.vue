...
const firstLevelOptions = computed(() =>
  firstLevelCategories.value.map(cat => ({ label: cat.categoryName, value: cat.categoryId }))
)
const secondLevelOptions = computed(() =>
  secondLevelCategories.value.map(cat => ({ label: cat.categoryName, value: cat.categoryId }))
)
